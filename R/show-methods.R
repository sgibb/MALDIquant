...
setMethod(f="show",
          ...
          definition=function(object) {
                 ...
                 ## to avoid newlines in other values don't format filenames and paths
...
setMethod(f=".prepareShow",
          ...
          definition=function(object) {
                 ...
                 if (isEmpty(object)) {
                        ...
                        values <- c(values,
                                    length(mass(object)),
                                    paste0(round(range(mass(object)), digits=3L), collapse=" - "),
                                    paste0(format(min(intensity(object)), digits=4L,
                                                  scientific=TRUE), " - ",
                                           format(max(intensity(object)), digits=4L,
                                                  scientific=TRUE)))
                 }
...
setMethod(f=".prepareShow",
          ...
          definition=function(object) {
                 ...
                 groups <- "Path to on-disk spectra"
